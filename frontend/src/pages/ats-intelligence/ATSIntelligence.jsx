...
function ATSIntelligence() {
  ...
  const keywordAnalysis = toObject(
    ats.keyword_analysis
  );

  const priorities = toObject(
    keywordAnalysis.priorities
  );
  ...
  const actionVerbs = toArray(
    ats.action_verbs_found
  );
  ...
  const technologyCoverage = toObject(
  ...
  const sectionAnalysis = toObject(
    ats.section_analysis
  );

  const sectionsPresent = toArray(
    sectionAnalysis.present
  );

  const sectionsMissing = toArray(
    sectionAnalysis.missing
  );

  const hasExperience = toBoolean(
    sectionAnalysis.has_experience
  );
  ...
  const deploymentAnalysis = toObject(
    ats.deployment_analysis
  );

  const applicationServing = toArray(
    deploymentAnalysis.application_serving
  );

  const productionDeployment = toArray(
    deploymentAnalysis.production_deployment
  );

  const hasApplicationServing =
    toBoolean(
      deploymentAnalysis.has_application_serving
    );

  const hasProductionDeployment =
    toBoolean(
      deploymentAnalysis.has_production_deployment
    );
  ...
  const scoringAnalysis = toObject(
    ats.analysis
  );

  const keywordStrength = toNumber(
    scoringAnalysis.keyword_strength,
    0
  );

  const sectionQuality = toNumber(
    scoringAnalysis.section_quality,
    0
  );

  const technicalContext = toNumber(
    scoringAnalysis.technical_context,
    0
  );

  const readability = toNumber(
    scoringAnalysis.readability,
    0
  );

  const contentImpact = toNumber(
    scoringAnalysis.impact_score,
    0
  );

  const contributionTotal = toNumber(
    scoringAnalysis.contribution_total,
    atsScore
  );

  const scoreConsistent = toBoolean(
    scoringAnalysis.score_consistent
  );

  const wordCount = toNumber(
    ats.word_count,
    0
  );
  ...
  const atsStatus = useMemo(() => {
    if (atsScore >= 85) {
      return {
        label: "Excellent",
        description:
          "Your resume is strongly optimized for automated screening systems.",
      };
    }
    ...
    if (atsScore >= 70) {
      return {
        label: "Good",
        description:
          "Your resume has a solid ATS foundation but still has optimization opportunities.",
      };
    }
    ...
    if (atsScore >= 55) {
      return {
        ...
        description:
          "Your resume may pass some automated screens, but important optimization gaps remain.",
      };
    }
    ...
    return {
      ...
      description:
        "Your resume has significant ATS weaknesses that could reduce screening performance.",
    };
  }, [atsScore]);
  ...
  const priorityGroups = [
    {
      ...
      label: "Critical",
      description:
        "Core technologies and competencies that strongly influence role relevance.",
    ...
    },
    {
      ...
      label: "High priority",
      description:
        "Frequently valuable skills that can materially improve matching.",
    ...
    },
    {
      ...
      label: "Important",
      description:
        "Relevant skills that can strengthen broader job compatibility.",
    ...
    },
    {
      ...
      label: "Supporting",
      description:
        "Additional technologies and tools that improve technical coverage.",
    ...
    },
    {
      ...
      label: "Optional",
      description:
        "Lower-priority terms that may provide additional role alignment.",
      data: toObject(priorities.optional),
    },
  ];

  // --------------------------------------------------------------------------
  // Keyword priority lookup
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  // Safe recommendation / issue values
  // --------------------------------------------------------------------------
      ...
      <div className="min-h-screen bg-[#050816] text-white">
        <div className="mx-auto flex min-h-screen max-w-4xl items-center justify-center px-6">
          <div className="w-full rounded-[32px] border border-white/[0.08] bg-[#0a1020] p-10 text-center shadow-2xl">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-blue-400/10 bg-blue-500/10 text-2xl text-blue-400">
              ◈
            </div>

            <h1 className="mt-6 text-2xl font-bold tracking-tight">
              ATS Intelligence is waiting
            </h1>

            <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-slate-500">
              Upload and analyze your resume first.
              ResumeIQ will then build your ATS
              intelligence profile using the analysis
              results.
            </p>
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
                Understand how automated hiring systems
                interpret your resume, where your
                keyword coverage is strong, and which
                optimization opportunities can improve
                your screening performance.
              </p>
            </div>
              ...
              <div className="flex flex-col gap-7 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <div className="mt-4 flex items-end gap-3">
                    ...
                    <span className="mb-2 text-lg text-slate-600">
                      / 100
                    </span>
                  </div>
        ...
        <section className="mb-6 rounded-[28px] border border-white/[0.08] bg-[#0a1020] p-6 sm:p-7">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <MiniMetric
              ...
              value={contributionTotal.toFixed(2)}
            />
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-[11px] leading-5 text-slate-600">
                  The values above are contribution signals
                  used by the ATS engine. They are not
                  independent percentages and should not be
                  interpreted as separate ATS scores.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="mb-6 rounded-[28px] border border-white/[0.08] bg-[#0a1020] p-6 sm:p-7">
          ...
          <div className="mt-5 rounded-2xl border border-amber-400/[0.08] bg-amber-400/[0.025] p-4">
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-300">
                  Missing does not mean mandatory
                </p>

                <p className="mt-1 text-[11px] leading-5 text-slate-600">
                  A keyword is marked as missing when the
                  ATS engine does not detect it in your
                  resume. Add it only if you actually have
                  relevant knowledge or experience. For
                  job-specific importance, use Job Matching
                ...
                description={
                  metrics.length
                    ? metrics
                        .slice(0, 8)
                        .join(", ")
                    : "No measurable outcomes detected"
                }
              />
                ...
                description={
                  technologies.length
                    ? technologies
                        .slice(0, 8)
                        .join(", ")
                    : "No technology coverage detected"
                }
              />
            </div>
          </div>
          ...
          <div className="rounded-[28px] border border-white/[0.08] bg-[#0a1020] p-6">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              <StructureCard
                label="Detected"
                value={sectionsPresent.length}
                items={sectionsPresent}
                positive
              />

              <StructureCard
                label="Missing"
                value={sectionsMissing.length}
                items={sectionsMissing}
              />
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Backend ATS section assessment
                  </p>
                </div>

                <span className="text-lg font-bold text-white">
                  {sectionQuality.toFixed(0)}
                </span>
              </div>
          ...
          {Object.keys(
            ...
            <div className="mt-6">
              <div className="mb-4">
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Technology signals grouped by ATS
                  category.
                </p>
              </div>
          ...
          <div className="mt-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-[11px] leading-5 text-slate-600">
                  A framework such as Flask can indicate
                  application serving, but it does not by
                  itself prove that the application was
                  deployed to production. Production
                  deployment should only be claimed when
                  the resume contains genuine deployment
                  evidence.
                </p>
              </div>
            </div>
          ...
          <div className="mt-6 space-y-3">
            {recommendationItems.length > 0 ? (
              ...
              <EmptyState
                ...
                description="Your current resume has a relatively healthy ATS profile."
              />
            )}
          </div>
        </section>
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-2">
              {issueItems.map(
                (issue, index) => (
                  <div
                    key={`${issue}-${index}`}
                    className="rounded-2xl border border-amber-400/[0.08] bg-amber-400/[0.025] p-4"
                  >
                    <div className="flex gap-3">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-2">
              {strengthItems.map(
                (strength, index) => (
                  <div
                    key={`${strength}-${index}`}
                    className="rounded-2xl border border-emerald-400/[0.08] bg-emerald-400/[0.025] p-4"
                  >
                    <div className="flex gap-3">
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-xl font-bold text-white">
                Turn ATS insights into job-specific intelligence
              </h2>

              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                Compare your resume against an actual job
                description to understand role-specific fit,
                keyword alignment, skill gaps, and
  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
      ...
      <div className="mt-5">
        {matched.length > 0 && (
          <div className="mb-4">
            <p className="mb-2 text-[9px] font-semibold uppercase tracking-wider text-emerald-500">
              Already covered
            </p>
        ...
        {missing.length > 0 && (
          <div>
            <p className="mb-2 text-[9px] font-semibold uppercase tracking-wider text-amber-500">
  ...
  return (
    <div className="rounded-[28px] border border-white/[0.08] bg-[#0a1020] p-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-lg bg-white/[0.03] px-3 py-1 text-[10px] font-bold text-slate-500">
          {items.length}
        </span>
      </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {values.map(
            (value, index) => (
              <span
                key={`${value}-${index}`}
                className="rounded-md bg-white/[0.035] px-2 py-1 text-[9px] text-slate-500"
              >
                {value}
              </span>
            )
      ...
      <div className="mt-5">
        {items.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {items.map(
              (item, index) => (
                <span
                  key={`${item}-${index}`}
                  className="rounded-lg border border-white/[0.06] bg-white/[0.03] px-3 py-2 text-[10px] font-medium text-slate-300"
                >
                  {item}
                </span>
              )
          ...
          <p className="text-xs leading-5 text-slate-700">
            No evidence detected in the resume.
          </p>
        )}
      </div>
    </div>
